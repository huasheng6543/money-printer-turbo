...
- **多源素材聚合**：新增 Pixabay API 搜索，素材库翻倍
- **HD质量过滤**：强制过滤 ≥1080px 宽度的高清视频
- **双重去重机制**：URL去重 + 基于汉明距离的视频内容指纹去重，避免重复视频
- **并发下载优化**：线程数从5增加到10，大幅提高下载速度
- **智能素材数量控制**：基于音频时长自动调整所需素材数量

### 📝 关键词生成增强

- **高维搜索词**：关键词数量从5个增加到15个，覆盖更多场景
- **多样化分类**：包含特写/广角/动作/抽象/不同角度等多种类型
- **时间线关键词**：新增时间线关键词匹配，实现视觉与音频同步

### 🎬 视频处理修复

- **MoviePy API兼容**：修复 `with_effects()`、`set_duration()` 等API调用，适配MoviePy 2.x版本
- **音频处理修复**：修复音频丢失问题，分离视频/音频处理流程
- **片段去重**：每个视频素材只使用一次，避免循环播放
- **资源管理优化**：添加 `close_clip()` 和 `delete_files()` 函数，及时清理临时文件

### 🗣️ 音频生成修复

- **edge-tts支持**：添加微软Edge语音合成作为fallback，提高音频生成成功率
- **SiliconFlow支持**：新增硅基流动语音合成支持，提供更多语音选择
- **语言检测**：添加语言检测和语音-语言匹配检查

### 📜 字幕修复

- **SubMaker修复**：修复 `offset` 属性不存在的问题，使用 `cues` 替代

### 🔧 配置优化

- **.gitignore更新**：添加临时文件、缓存目录、环境文件等忽略规则
