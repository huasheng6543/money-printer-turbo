...
def get_audio_duration(sub_maker: submaker.SubMaker):
    ...
    获取音频时长
    """
    # 新版 edge-tts 使用 cues 属性，旧版使用 offset 属性
    if hasattr(sub_maker, 'offset') and sub_maker.offset:
        return sub_maker.offset[-1][1] / 10000000
    if hasattr(sub_maker, 'cues') and sub_maker.cues:
        # cues 是字幕时间戳列表，最后一个元素的结束时间就是音频时长
        last_cue = sub_maker.cues[-1]
        # cues 的时间格式可能是 timedelta 或秒数
        if hasattr(last_cue, 'end'):
            end_time = last_cue.end
